...
# openssl x509 -in cacert.crt -inform PEM -outform DER| hexdump -v -e '1/1 "\\\x"' -e '1/1 "%02X"' > my-cert-hexesc.der
#
import getopt
import sys
import os
...
def convert_cert(infile,openssl="/usr/bin/openssl", hexdump="/usr/bin/hexdump") :
    subject = None
    hexcrt = None
    bro_stanza = None
    ...
    rsub = os.popen("%s x509 -in %s -inform PEM -noout -subject" % (openssl,infile))
    ...
    if (subject is None):
        raise Exception, "Unable to parse a certificate subject out of %s" % infile
    
    rhex = os.popen("%s x509 -in %s -inform PEM -outform DER| %s -v -e '1/1 \"\\x\"' -e '1/1 \"%%02X\"'" % (openssl,infile,hexdump));
    for rhexline in rhex:
        m = re.search('([x,0-9,A-F]+)',rhexline)
        if (m != None) :
            hexcrt = "\\x".join(m.group(1).split("x"))
            
    ## I don't know if it's possible to not have a value here, but I'd rather be sure.
    if (hexcrt):
