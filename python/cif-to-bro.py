...
def get_cif_domains() :
    rclient = cif.ClientINI(path='/usr/local/etc/cif.cfg',fields=['address','description'],no_verify_tls=True)
...
def bro_format(domain, category=None, source="CIF", description=None, severity=None,raw=None, confidence=None):
	fmt = "[\"%s\"] = [ " % domain
        if (category):
...
def main() :
	...
	for dom_ent in doms.splitlines():
		...
		if description.find("tdl") >= 0 or description.find("tdss") >= 0 :
			bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="TDSS",description=description))
		elif description.find("zeus") >= 0:
			bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="Zeus",description=description))
		elif description.find("torpig") >= 0:
			bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="Torpig",description=description))
		elif description.lower().find("bredolab") >= 0:
                        bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="Bredolab",description=description))
                elif description.find("mebroot") >= 0:
                        bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="Mebroot",description=description))
                elif description.lower().find("virut") >= 0:
                        bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="Virut",description=description))
		elif description.lower().find("gbot") >= 0:
                        bro_file.write(bro_format(address[:-1],raw=dom_ent[:-1],category="GBot",description=description))
#		else:
